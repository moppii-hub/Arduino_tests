...
// If you want, can add LPF at D11 output.
// D11 -> 1kOHM -> Speaker
//           |-> 100nF -> GND
...
float volume[4] = {0, 0.25, 0.5, 0.75};
byte vol_step = 0;
uint16_t timer_count_vol = 1;
...
ISR(TIMER1_COMPA_vect) {            //call rate = 10kHz
    ...
    OCR2A = sinetable[timer_sample] * volume[vol_step];

    timer_count++;
    if(timer_count > timer_count_limit) timer_count=0;

    timer_count_vol++;
    if(timer_count_vol % 10000 == 0){
        vol_step=(vol_step+1)%4;
        timer_count_vol = 1;
    }
}
